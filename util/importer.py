...
def importCsv(path, isotope="Unknown", config=None, calibration=None, read_config=True):
        ...
        if line == "# Pew Pew Export":  # CSV generated by pewpew
            ...
            if read_config:
                ...
                if ';' not in line or '=' not in line:
                    raise PewPewConfigError("Malformed config line.", line)
                for token in line.split(";"):
                    k, v = token.split("=")
                    if k not in config.keys():
                        raise PewPewConfigError("Invalid key.", k)
                    try:
                        config[k] = float(v)
                    except ValueError as e:
                        raise PewPewConfigError("Invalid value", v) from e
    ...
    if data.ndim != 2:
        raise PewPewDataError("Invalid data dimensions.", data.ndim)
